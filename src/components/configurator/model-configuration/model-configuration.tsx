...
function Model(props: any) {
    ...
    if (parent.children.length == 0) {
      geometry.push(
        <mesh
          ...
          material-color={snap.items[parent.name]}
        />
      );
    } else {
  ...
  return (
    <group ref={ref} castShadow receiveShadow dispose={null}>
      {geometry && geometry.map((mesh: any) => mesh)}
    </group>
  );
}
...
class ModelConfiguration extends React.Component<any, any> {
  constructor(props: any) {
    ...
    this.updateOptionsPrice = this.updateOptionsPrice.bind(this);

    const id = parseInt(props.match.params.id ?? 0);

    state.items.same = props.hasFurniture;
  ...
  async fetchModel() {
    if (this.state.editMode) {
      ...
      let mapped: any = await Promise.all(
        conf?.configurationValues?.map(async (element: any) => {
    ...
    const file: any = await fetch(
      `${REACT_APP_API_BASE_URL}/${objectJson.value}`,
      {
        method: 'GET',
      }
    )
  ...
  updateOptionsPrice() {
    const { optionValues: ov } = this.props;
    const { options, savedValue } = this.state;

    const optionValues: any = (ov?.length > 0 ? ov : savedValue).filter(
      (value: any) => value
    );

    if (options?.length > 0 && optionValues?.length > 0) {
      const values = options.flatMap((option: any) => option.values);

      let optionsPrice = 0;

      optionValues.forEach(
        (id: number) =>
          (optionsPrice += parseFloat(
            values.find((val: Value) => val.id === id)?.price ?? 0
          ))
      );
  ...
  handleChangeCB(event: any) {
    ...
    this.props.updateHasFurniture(val as boolean);
  }
